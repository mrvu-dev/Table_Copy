class SelectionWorkshop {
  ...
  renderRecipeMode(recipe) {
    ...
    this.renderDataPreview_(this.recipeWrapper_.getAsArrays())
      ...
      .finally(() => this.handleRenderCompletion_());
  }
  ...
  handleRenderCompletion_() {
    this.frame_.setLoading(false);
    // This is useful for iframed pages where the workshop is fixed beneath the fold.
    this.frame_.scrollIntoView();
  }
  ...
  renderSelectionMode() {
    ...
    if (this.userConfig_.requiresPaid && !this.userConfig_.paidProOrMore) {
    ...
    if (this.selectionNode_ && _tcIsNodeWithinTable(this.selectionNode_)) {
      ...
      return this.fetchTableData_()
        ...
        .then(() => this.handleRenderCompletion_())
        .catch((err) =>
  ...
  renderContentFrame_() {
    ...
    this.contentWrapper_.innerHTML = `
      <div class="tc-col _tc-left">
        ...
        <div class="_tc-special-table-ads tc-hidden"></div>
      </div>
    ...
    const actionDefs = {
      ...
      "btn-csv": {
        ...
        disabled: !this.userConfig_.paidProOrMore,
      },
      "btn-excel": {
  ...
  renderSpecialPageCTAs_() {
    const wrapper = this.contentWrapper_.querySelector(
      "._tc-special-table-ads"
    );
    const { isDunePage } = checkIsSpecialPage();
    if (isDunePage) {
      wrapper.classList.remove("tc-hidden");
      wrapper.innerHTML = `
        <div class="_tc-dune-cta">
          <a href="${_TCAP_CONFIG.duneExtension}" target="_blank">
            Export Dune query results instantaneously with the <b>Dune Data Extractor by Table Capture</b>.
          </a>
        </div>
      `;
    }
