...
function _tcGetCurrentPageNumber() {
  try {
    ...
    if (search) {
      const matches = search.match(/page=([0-9]+)/);
...
function _tcWaitForElement(selector, maxWait) {
  ...
  if (maxWait < 0) {
    return Promise.reject(`Element not found for selector: ${selector}`);
  }
...
function _tcGetRepresentativeRow(dataArray) {
  ...
  let firstUsefulRow = dataArray[0].filter((c) => c && c.trim());
  let i = 0;
  while (
    firstUsefulRow &&
    ...
    i++;
    firstUsefulRow = dataArray[i].filter((c) => c && c.trim());
  }

  // This happens if we couldn't find anything.
...
function _tcArbNodeToArray(
  ...
  if (node.childNodes.length) {
    ...
    const val = _tcTextNodeVal(node, false, redGreenExtract).trim();
    currentRow.push({ val, path });
...
function _tcIsInRedSpace(r, g, b) {
  return r > g && r > b && r - g > 50 && r - b > 50;
}

function _tcIsInGreenSpace(r, g, b) {
  // Check if the color is in the green space
  return g > r && g > b && g - r > 50 && g - b > 50;
}

function _tcTextNodeVal(
  textNode,
  newLinesAsSpaces = false,
  redGreenExtract = false
) {
  const re_n = new RegExp("\\n", "g");
  const re_t = new RegExp("\\t", "g");

  let tempStr = textNode.nodeValue;
  ...
  if (redGreenExtract && tempStr && textNode.parentElement) {
    try {
      // Get the font color.
      ...
      if (_tcIsInRedSpace(r, g, b)) {
        tempStr += " (red)";
      } else if (_tcIsInGreenSpace(r, g, b)) {
        tempStr += " (green)";
      }
    } catch (err) {}
  }
...
function _tcNodeToString(
  ...
  if (node.childNodes.length) {
    ...
    str += _tcTextNodeVal(node, newLinesAsSpaces, redGreenExtract).trim();
  }
